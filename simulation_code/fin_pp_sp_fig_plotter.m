...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Repeat pulse rate modulation with respect to HV-pulse rate conversion: 6/3/21
%%Last updated 7/2/21
...
shadedErrorBar(1:size(prm_n,2),50*(mean(prm_n,1) - mean(prm_n(:)))/max(mean(prm_n,1)),std(prm_n,[],1)/max(mean(prm_n,1)),'lineProps',{'r-'}); hold on;
...
subplot(4,3,n);
%suptitle('PAM')
 shadedErrorBar(1:size(prm_n,2),mean(pr_pam_n,1)-mean(pr_pam_n(:)) ,std(pr_pam_n,[],1),'lineProps',{'k-'}); hold on;
 shadedErrorBar(1:size(prm_n,2),50*(mean(pam_n,1) - mean(pam_n(:)))/max(mean(pam_n,1)),std(pam_n,[],1)/max(mean(pam_n,1)),'lineProps',{'b-'}); hold on; 
