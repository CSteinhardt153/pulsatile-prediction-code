...
%Table of afferent specific data ( from past recordings):
%Aff num; kaiser thresh min/max; pulse artifact window 
...
cur_timing = aff_dat.t;
if min(unique(diff(cur_timing))) > 1e-4
    cur_timing = aff_dat.t*1e-3;
end

info_row = find(aff_sort_setting(:,1) == aff_num);
k_thresh_min_max = aff_sort_setting(info_row,2:3);
pulse_art_wind = aff_sort_setting(info_row,4);

% Look at window outside pulse time
% Set threshold min and max - find the Kaiser filter timing that shows
spike_info = find_AP_times(cur_timing,raw_spiking,stim_trace, ...
    k_thresh_min_max,pulse_art_wind,32,plot_steps)


%Sorted spikes here:
figure(12);
plot(cur_timing, raw_spiking,'k'); hold on;
plot(cur_timing(spike_info.pulse_evoked_spk_idxs),raw_spiking(spike_info.pulse_evoked_spk_idxs),'m.');
plot(cur_timing(spike_info.spont_spk_idxs),raw_spiking(spike_info.spont_spk_idxs),'g.');

% Check induced firing rate:
%
%%
[avg_frs, pr_info, spont_win, time_ds] = sort_spk_to_pfr(spike_info,cur_timing)
...
%%%% Point of this section is to try to align to right time on pulse as
%%%% much as possible to be more likely to find the peak later
...
% %Check kaiser spike detection for spontaneous spikes:
if plot_it
    figure(9);
...
spk_times_tmp = diff(spk_time_tmp_2);
...
post_pulse_window = 100; % works across both cases
...
%Add test of about how big spont spikes would be with this normalization:
%%%Not much variance in pulse height, so:

clear spnt_wind spnt_orig_wind
%Make sure don't count artifact:
max_spnt = 1000;
for n_spnt = 1:length(spont_idxs)
    rel_idxs = max(1,(spont_idxs(n_spnt)-50):(spont_idxs(n_spnt)+40));
    [val_m idx_m] = max(cur_spiking(rel_idxs));
    corr_idx(n_spnt,:) = max(1,(rel_idxs(idx_m)-50):(rel_idxs(idx_m)+40));
    spnt_orig_wind(n_spnt,:)= cur_spiking(max(1,(spont_idxs(n_spnt)-50):(spont_idxs(n_spnt)+40)));
end

comp_spspk_norm = mean(max(zmean_resp,[],2) - min(zmean_resp,[],2));
%figure(8); plot(spnt_orig_wind');%spnt_wind');
if plot_it
figure(10);
%for n_spnt = 1:length(spont_idxs)
   plot(-50:40,spnt_orig_wind'./comp_spspk_norm); title(num2str(cur_timing(corr_idx(n_spnt,1)))); hold on;
   %pause;  
%end
end
...
function [avg_frs, pr_info, spont_win, time_ds] = sort_spk_to_pfr(spike_info,cur_timing)
%Turn sorted spikes into firing rate:
%Find simulation blocks
