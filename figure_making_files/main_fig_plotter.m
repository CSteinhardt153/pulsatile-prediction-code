...
% figure(1); 
% subplot(2,1,1);
% plot(p_times,ones(size(p_times)),'k|'); hold on;
% plot(spk_times,ones(size(spk_times)),'r|')
% subplot(2,1,2);
% mod_spk_t = mod(spk_times-p_times(1),mean(out.spiking.end.ISI));
% histogram(mod_spk_t)
% xlim([0 mean(out.spiking.end.ISI)])
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%RUN EXPERIMENTS FROM HERE ('exact') settings are in this code:
run_chosen_expt(expt,run_mode,override,output);
%% Comparison of Regular v. Irregular: 
% Figure 5 D ==============================================
output.vis_plots = 1;
run_mode = 'override';
expt.num = [];%[3];
inj_cur = [1 1];
pulse_rate = [100 150]%[0:2:360]; %[225 250 275 300 325];%[25:25:300]; %180;%%[50 80 120 150 200];%100 150 200 250 300 ]%0:5:350;%[0 25 50 200];
...
figure(10); 
subplot(2,1,n);
mod_spk_t = mod(spk_times-strt_p,isi);
xlabel('t (ms) from Pulse'); ylabel('Num APs')
...
[override] = set_overrides_v2(run_mode,output,{'pulse_rate',pulse_rate },...
   ...
   {'gNas',13},{'gKHs',2.8},{'gKLs',1},{'epsc_scale',1},{'mu_IPT',1.3},{'sim_time',5000});%3});
disp('Ireg: ')
disp([ override.gNas override.gKHs override.gKLs override.sim_info.mu_IPT])
out= run_chosen_expt(expt,run_mode,override,output);
%Spike times compared to Pulse times
...
for pulse_rate = [100 150]
[override] = set_overrides_v2(run_mode,output,{'pulse_rate',pulse_rate },...
    {'curr_options',curr_options},{'inj_cur',inj_cur},{'is_reg',1},...
   {'gNas',13},{'gKHs',2.8},{'gKLs',0},{'epsc_scale',0.025},{'mu_IPT',0.09},{'sim_time',5000});%3});
disp('Ireg: ')
disp([ override.gNas override.gKHs override.gKLs override.sim_info.mu_IPT])
out= run_chosen_expt(expt,run_mode,override,output);
%Spike times compared to Pulse times
...
irreg = load(fullfile(data_dir,'specific_runs','fr_reps_3_steps_10-May-2021_reg_0_spking_1.mat'));
%load('fr_reps_5_steps_07-May-2021_reg_0_spking_1.mat');
reg1  = load(fullfile(data_dir,'specific_runs','fr_reps_3_steps_10-May-2021_reg_1_spking_1.mat'));
reg2  = load(fullfile(data_dir,'specific_runs','fr_reps_2_steps_10-May-2021_reg_1_spking_1.mat'));
reg3  = load(fullfile(data_dir,'specific_runs','fr_reps_1_steps_10-May-2021_reg_1_spking_1.mat'));
%%
...
%% See whether jitter distorts these effect with jitter of 2 ms around pulse time - 1 second long simulations
 q =load(fullfile(data_dir,'specific_runs','pr_fr_sim_rep5_I0-360_PR0-360_S84.1366_reg0_05_23_2021_15_18.mat')) % here Jitter = 2 ms std
 figure(100);
 ...
 %============= Other Experiments not in current version of paper ================
...
sim_wind_dir = fullfile(data_dir,'specific_runs','10_rep_sim_winds');
cd(sim_wind_dir);
...
%% 5 is the largest time window - like typical run
%Checking whether the shapre of the PFR is different - less bumpy etc is
%window decreases:
for n_ax  =1:6
    idx_ord(n_ax)
...
run_chosen_expt(expt,run_mode,override,output);
%========================================================================
%% Pulse rate/amplitude modulation experiments:
...
%% Behavioral map given the PFR with spotaneous activity:
%Figure 5 A ======================================================
...
%% Plot jitter: from saved jitter experiments
%Figure 5 E
%======================================================================
jit_dat= fullfile(data_dir,'specific_runs/simwindowandjitter');
dat = load(fullfile(jit_dat,'jitter_2_pr_fr_sim_rep5_I0-360_PR0-360_S0_reg0_05_24_2021_13_15.mat'));
dat2 = load(fullfile(jit_dat,'jitter_1_pr_fr_sim_rep5_I0-360_PR0-360_S0_reg0_05_24_2021_11_33.mat'));
data_dir_higher_axon = fullfile(data_dir,'specific_runs/3_axon_runs_4_12_21/axon_1_dS_PR_FR_data');
higher_I = load(fullfile(data_dir_higher_axon ,'pr_fr_sim_I0-360_PR0-350_MInf_S0_R5_04_10_2021_09_17.mat'))'
...
%% Extra section for trying different pulsatile waveforsm
